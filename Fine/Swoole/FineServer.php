...
use Fine\ConfigTrait;
use Fine\Swoole\Server\HttpServer;
use Fine\Swoole\Server\UdpServer;
...
class FineServer
{
    ...
    public static function runAll()
    {
        if (self::$_server === null) {
            ...
            self::$_server = $server;
            self::e('server start');
            self::e('fine version : ' . _Fine_V_);
            @swoole_set_process_name('fine_master_' . md5(serialize(self::$conf)));
            $server->start();
        }
    }
    ...
    private static function onEvent($sev, $class, $server, $conf, $call = [])
    {
        ...
        foreach ($funcs as $func) {
            if (strpos($func->class, '\\Swoole\\') === false) {
